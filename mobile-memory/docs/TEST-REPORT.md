...
## 十、v1.2 增补:修改密码 + 背词/单词本对齐 + 用户反馈批量修复(2026-09-18)

新增:
- 修改密码(我的页-账号安全:新密码+确认,PUT /api/password;退出后用新密码登录验证通过)
- 服务端 review/today:队列卡词自动并入单词本(幂等)——修复背词有词而单词本为空的数据不一致

用户反馈批量修复:
- 首页搜索框从底部改为垂直居中(查词后回到顶部)
- 去掉「加入单词本」按钮(查词默认自动加入),发音改为词旁扬声器图标
- 英文释义按词性分段分行(修复 institution 释义一大坨+孤立 n. 问题),超长折叠
- 例句超长截断(line-clamp)
- 词形变化改为「标签+词」分离展示(如 plural / institutions)
- 左滑删除:按钮全宽吸附(84px)、颜色改更鲜艳的 #ef4444
- 单词本/背词页顶栏加返回首页按钮
- 每日目标在统计行内可改(调整-输入-失焦/回车保存)

已验证:改密码-退出-新密码登录闭环;对齐逻辑(清空单词本-review/today 自动补回 14 词);每日目标保存。
真机项:头像文件选择、键盘搜索键、小组件。
